...
import { CreateNewShortenedUrlDialog } from "..";
import { useDispatch, useSelector } from "react-redux";
import { setAuthFormsDialogActive } from "@/redux/slices/ui_slice";
import { useNavigate, useSearchParams } from "react-router-dom";

// Add dialog which will open when user paste url and click on 'shorten now' button.
// Create 'url-shortener-form' component to add in dialog.

const UrlShortenerInputForm = ({ type = "dashboard", className }) => {
  ...
  return (
    <>
      <CreateNewShortenedUrlDialog
        ...
        longLink={form?.getValues("longURL")}
      />
      ...
      <div className="flex flex-col gap-5">
        <Form {...form}>
          <form
            ...
            className={`sm:h-20 h-[60px] w-full ${
              type !== "dashboard" && "max-w-[650px]"
            } mx-auto py-2.5 px-1.5 flex items-center gap-2.5 border-4 border-border sm:rounded-4xl rounded-3xl bg-card focus-within:ring-2 focus-within:ring-ring/80 transition-[color,box-shadow] ${className}`}
          >
        ...
        <div
          className={`flex items-center gap-2 ${
            type === "dashboard"
              ? "sm:justify-between justify-center flex-wrap"
              : "flex-col"
          }`}
        >
          <p className="text-sm">
...
export default UrlShortenerInputForm;
